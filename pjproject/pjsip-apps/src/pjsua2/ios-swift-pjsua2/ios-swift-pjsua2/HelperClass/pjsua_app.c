...
//void  currentCallNotification(char* callStatus,int callid,char* calleeNum,pjsua_call_info callInfo);
void displayPreviewWindow(pjmedia_vid_dev_index wid,pj_bool_t isPreview,pjsua_call_id call_id);
void  initAudiRingTone(pjsua_app_config config);
#endif
...
static void init_ringtone_player() {
    ...
    if (status != PJ_SUCCESS) {
            PJ_LOG(3,(THIS_FILE,"Error creating WAV player port %@",status));
        return;
    }
    ...
    if (status != PJ_SUCCESS) {
            PJ_LOG(3,(THIS_FILE,"Error adding port to conference %@",status));
        return;
    }
...
//    currentCallNotification((char*)("DISCONNCTD"),call_id,call_info.remote_info.ptr,call_info);
            printf("DISCONNECTED........");
        
//        sip_ring_stop(&app_config, call_id);
        ...
        if (call_info.state == PJSIP_INV_STATE_CALLING) {
         ...
         //   currentCallNotification((char*)("CALLING"),call_id,call_info.remote_info.ptr,call_info);
        ...
        if (call_info.state == PJSIP_INV_STATE_EARLY) {
            ...
            if(code == 180 || code == 183){
              //  currentCallNotification((char*)("RINGING"),call_id,call_info.remote_info.ptr,call_info);
//                sip_ring_init(&app_config);
//                    sip_ring_start(&app_config, call_id);
            }
        ...
        //    currentCallNotification(call_info.state_text.ptr,call_id,call_info.remote_info.ptr,call_info);
//            sip_ring_stop(&app_config, call_id);
        }
...
//currentCallNotification((char*)("INCOMING"),call_id,call_info.remote_info.ptr,call_info);
    
#ifdef USE_GUI
    if(current_call!=PJSUA_INVALID_ID){
        pjsua_call_answer(call_id, 180, NULL, NULL);
        printf("INCOMING CALL:remote_info: %s",call_info.remote_info.ptr);

        pj_str_t from =  call_info.remote_info;
//        if (!showNotification(call_id,from))
//        return;
    }else{
...
//             pj_get_version(), PJ_OS_NAME);
    ...
    pj_ansi_snprintf(tmp, sizeof(tmp), "%s",PJ_OS_NAME);
