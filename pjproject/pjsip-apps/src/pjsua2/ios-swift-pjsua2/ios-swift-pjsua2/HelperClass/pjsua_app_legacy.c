...
void connectMedia()
{
    ...
    pj_status_t status5 = pjsua_conf_connect(ci.conf_slot,call_info.conf_slot);
    
    pj_status_t status6 = pjsua_conf_connect(call_info.conf_slot,ci.conf_slot);
    printf("status6 status %d\n",status6);
    printf("status5 status %d\n",status5);
...
pj_bool_t getRegisterStatus(){
    pjsua_acc_info reg_info;
    pj_bool_t status;
    do{
        pjsua_acc_get_info(current_acc,&reg_info);
        
        if(reg_info.status==200 && (reg_info.expires)){
            status=PJ_TRUE;
            char state[100];
            sprintf(state,"%d %s", reg_info.status,pj_strbuf(&reg_info.status_text));
            //setRegisterStatus((char*)state);
            printf("The Last Registration Status received From Server is :  %d \n",reg_info.status);
            // printf("The Last Registration Status received From Server is :  %s \n",reg_info.status_text);
            printf("if status returning:%d",status);
            return status;
            ...
            if(reg_info.status!=100){
                printf("The Last Registration Status received From Server is :  %d \n",reg_info.status);
                //printf("The Last Registration Status received From Server is :  %s \n",reg_info.status_text);
                printf(" else status returning:%d",status);
                if(reg_info.status!=401){
                    printf("The Last Registration Status received From Server is :  %d \n",reg_info.status);
                    //printf("The Last Registration Status received From Server is :  %s \n",reg_info.status_text);
                    printf(" else status returning:%d",status);
                    
                    /* Setting failure Registration status */
                    char state[100];
                    sprintf(state,"%d %s", reg_info.status,pj_strbuf(&reg_info.status_text));
                    printf("print state %s",state);
                    //setRegisterStatus((char*)state);
                    return status;
                }
                char state[100];
    ...
    }while(reg_info.status==100);
    printf("The Last Registration Status received From Server is :  %d \n",reg_info.status);
    printf(" after while status returning:%d",status);
    
    return status;
    
}

pj_status_t addAccount(pj_str_t sipurl,pj_str_t registarurl,pj_str_t uname,pj_str_t passwd,pj_str_t realm)
{
    ...
    pjsua_acc_config_default(&acc_cfg);
    acc_cfg.id = sipurl;
    acc_cfg.reg_uri = registarurl;
    acc_cfg.cred_count = 1;
    acc_cfg.cred_info[0].scheme = pj_str("Digest");
    acc_cfg.cred_info[0].realm = realm;
    acc_cfg.cred_info[0].username = uname;
    acc_cfg.cred_info[0].data_type = 0;
    acc_cfg.cred_info[0].data = passwd;
    //acc_cfg.reg_retry_interval = 60;
    acc_cfg.reg_timeout=60; //Expires Header Value For Registration.
    acc_cfg.reg_delay_before_refresh=20;//Sending Re-Reg before 20Sec from Expires Value.
    
    
    //configuration by Nexge.
    acc_cfg.use_rfc5626=PJ_FALSE;   //remove Route headers, remove contact ob parameter.
    acc_cfg.use_timer=PJSUA_SIP_TIMER_INACTIVE; //disable session expires.
    ...
    status = pjsua_acc_add(&acc_cfg, PJ_TRUE, &reg_acc_id);
    
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Error adding new account", status);
    }
    
    printf("REGISTRATION ACCOUNT ID IS :::: %d\n",reg_acc_id);
    return status;
}
...
void delete_account()
{
    ...
    if (!pjsua_acc_is_valid(i)) {
        ...
        pjsua_acc_set_registration(i, PJ_FALSE); // to un-register the account.
        pjsua_acc_del(i);
        printf("Account %d deleted\n", i);
    }
//        pj_status_t status = PJ_SUCCESS;
...
//            status = pjsua_acc_del(current_acc);
//            if (status == PJ_SUCCESS){
//                printf("Account %d deleted\n", current_acc);
//            }
//        }
